...
def load():
        ...
        if "gmail.com" in returned:
            def checkgooglemail(): # function to check gmail email
                if "(1366, 768)" in screensizeout:
                    print(cwd)
                    print("success screensize is correct, using folder 1366x768 for locateonscreen function")
                    gmailfulllogin = (cwd+ '/1366x768/windows/gmailalreadyloggedin.png')
                subprocess.Popen(["C:\Program Files\Mozilla Firefox/firefox.exe", "-private-window", "https://accounts.google.com/AccountChooser/identifier?service=mail&continue=https%3A%2F%2Fmail.google.com%2Fmail%2F&flowName=GlifWebSignIn&flowEntry=AddSession"])
                ...
                gmailemail = pyautogui.locateCenterOnScreen(cwd+ '/1366x768/windows/gmailemail.png')
    ...
    def shutdowntime(): 
        while True:
            try:
                shutdowntime = int(gui_input('Please enter amount of seconds from now to schedule shutdown'));
                str(shutdowntime)
            except ValueError:
                print("Not a valid number")
                continue
                ...
                break
        process = subprocess.Popen(["shutdown", "/s", "/t", str(shutdowntime)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding='utf8')
        print(process)
        time.sleep(5)
    ...
    def restarttime():
        while True:
            try:
                restarttime = int(gui_input('Please enter time from now in seconds to schedule restart'));
                str(restarttime)
            except ValueError:
                print("Not a valid number")
                continue
        ...
        print(restarttime)
        process = subprocess.Popen(["shutdown", "/r", "/t", str(restarttime)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding='utf8')
        print(process)
        time.sleep(5)
        output = process.communicate()[0].split('\n')
        ...
        for data in output:
            if 'A system shutdown has already been scheduled.(1190)' in data:
                ...
                root.wm_title("Automate It - Restart Schedule Info")
                Label = tk.Label(root, font = ('Comic Sans MS',10))
                Label.pack()
                Label2 = tk.Label(root, text = "Restart has already been sch eduled '\n' please use cancel command first then try again", font = ('Comic Sans MS ',10), fg="green")
                Label2.pack()
                logging.info("Restart has already been scheduled, please use cancel command then run again")
    ...
    def ipconfig():
        process = subprocess.Popen(["ipconfig", "/release"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding='utf8')
        process2 = subprocess.Popen(["ipconfig", "/renew"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding='utf8')
        

         #Launch the shell command:
        output = process.communicate()[0].split('\n')
        output2 = process2.communicate()[0].split('\n')
        print (output)
        print (output2)

        for data in output2:
            if 'IPv4 Address. . . . . . . . . . . : 192' in data:
                root = tk.Tk()
                root.resizable(False, False)
                root.geometry("400x100")
                root.wm_title("Automate It - Restart Schedule Info")
                Label = tk.Label(root, font = ('Comic Sans MS',10))
                Label.pack()
                Label2 = tk.Label(root, text = "IP Address released and renewed", font = ('Comic Sans MS ',10), fg="green")
                Label2.pack()
                logging.info("IP Address Released and Renewed")
    ...
    def customfunctiontwo():
        os.system("customfunction2.py")

    def customfunctionthree():
        os.system("customfunction3.py")

    def customfunctionfour():
        os.system("customfunction4.py")

    def customfunctionfive():
        os.system("customfunction5.py")
    ...
    def websiteip(): # website IP function
        website = gui_input("Please enter the domain name of the site you would like to check IP www." )# prompt user for domain name
        ...
        root.resizable(False, False)
        root.geometry("400x180")
        root.wm_title("Automate It - Website IP Check")
